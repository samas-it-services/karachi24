...
```bash
k24GetData$ sam build --use-container
```
...
```bash
k24GetData$ sam logs -n StockCheckerFunction --stack-name k24GetData --tail
```
...
```bash
k24GetData$ pip install pytest pytest-mock --user
k24GetData$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name k24GetData
```
